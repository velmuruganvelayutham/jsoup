...
public class SportsTailgate implements Scraper {
	...
	public File extract() throws IOException, FailingHttpStatusCodeException,
		...
		File file = File.createTempFile(SportsTailgate.class.getName(), "csv");
		...
		writer.append('\n');
		String exhibitorUrl = "http://n2a.goexposoftware.com/events/sl2015/goExpo/public/listExhibitorsFrame.php";
		Document Showdoc = Jsoup.connect(exhibitorUrl).get();
		Elements links = Showdoc.select("a[href*=id]");
		int i = 0;
		for (Element link : links) {
			i++;
			writer.append("SPORTS LICENSING AND TAILGATE SHOW 2015");
			writer.append(",");

			writer.append("January 14-16 2015");
			writer.append(",");
